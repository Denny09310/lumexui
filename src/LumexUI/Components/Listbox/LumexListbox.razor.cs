...
public partial class LumexListbox<TValue> : LumexComponentBase, ISlotComponent<ListboxSlots>
{
	...
	{
		var listbox = Styles.Listbox.Styles( TwMerge );
		return listbox();
	}
	...
	private string? GetStyles( string slot )
	{
		...
		return slot switch
		{
			nameof( ListboxSlots.Base ) => style( Classes?.Base, Class ),
			nameof( ListboxSlots.List ) => style( Classes?.List ),
			nameof( ListboxSlots.EmptyContent ) => style( Classes?.EmptyContent ),
